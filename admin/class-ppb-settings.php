 ...
 *
 * Onglets :
 *  - Portail Partenaire : mot de passe, page portail, TTL token, logo, etc.
 *  - Catalogue Public   : page catalogue ([ppb_catalog])
 */
class PPB_Settings {

    private const MENU_SLUG           = 'ppb-settings';
    private const OPTION_GROUP        = 'ppb_options';
    private const OPTION_GROUP_CATALOG = 'ppb_catalog_options';
    ...
    public function render_settings_page(): void {
        ...
        $catalog_url     = $catalog_page_id > 0 && get_post( $catalog_page_id )
        ...
        <div class="wrap">
            ...
            <?php if ( 'catalog' === $current_tab ) : ?>
                ...
                <?php if ( $catalog_url ) : ?>
                <div class="ppb-inline-help ppb-inline-help--info" style="margin-bottom:16px;">
                    <strong><?php esc_html_e( 'Catalogue actif :', 'presellia-partner-bridge' ); ?></strong>
                    <a href="<?php echo esc_url( $catalog_url ); ?>" target="_blank"><?php echo esc_html( $catalog_url ); ?></a>
                </div>
                <?php endif; ?>

                <form method="post" action="options.php">
                    <?php settings_fields( self::OPTION_GROUP_CATALOG ); ?>

                    <h2><?php esc_html_e( 'Catalogue public', 'presellia-partner-bridge' ); ?></h2>
                    <p class="description" style="margin-bottom:12px;">
                        <?php esc_html_e( 'Le shortcode [ppb_catalog] affiche les produits avec leurs prix publics et leur disponibilité — sans mot de passe. Placez-le sur une page dédiée accessible à tous (service client, clients réguliers, etc.).', 'presellia-partner-bridge' ); ?>
                    </p>
                    <table class="form-table">
                        <tr>
                            <th><?php esc_html_e( 'Page catalogue', 'presellia-partner-bridge' ); ?></th>
                            <td>
                                <select name="ppb_catalog_page_id">
                                    <option value="0"><?php esc_html_e( '— Sélectionner une page —', 'presellia-partner-bridge' ); ?></option>
                                    <?php foreach ( $pages as $page ) : ?>
                                        <option value="<?php echo esc_attr( $page->ID ); ?>" <?php selected( $catalog_page_id, $page->ID ); ?>>
                                            <?php echo esc_html( $page->post_title ); ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                                <p class="description">
                                    <?php esc_html_e( 'Page sur laquelle vous avez placé le shortcode [ppb_catalog]. Les scripts du catalogue seront chargés uniquement sur cette page.', 'presellia-partner-bridge' ); ?>
                                </p>
                            </td>
                        </tr>
                    </table>
